...
float lightRange()
{
	...
	return -u_light.attenuation.linear + 
		sqrt(u_light.attenuation.linear * u_light.attenuation.linear - 4.0 * u_light.attenuation.quadratic * 
...
void main()
{
	...
	vec3 ambient = u_light.ambience.rgb;
	vec3 specular = specularCoefficient * u_light.intensities.rgb * vec3(1, 0, 0);
